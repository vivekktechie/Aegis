...
const ManageSessions: React.FC<ManageSessionsProps> = ({ guideId }) => {
  ...
  const fetchSessions = async () => {
    if (!guideId) return;
    try {
      const res = await axios.get(`/api/sessions/guide/${guideId}`);
  ...
  const handleCreateSession = async () => {
    if (!title || !description || !meetingLink) return;
    setLoading(true);
    try {
      await axios.post('/api/sessions', { title, description, meeting_link: meetingLink, guide_id: guideId });
      setTitle('');
      setDescription('');
      setMeetingLink('');
      fetchSessions();
  ...
  return (
    <div className="mb-5">
      <h4 className="mb-3">Your Q&A Sessions</h4>
      ...
      {sessions.length === 0 ? (
        ...
        <div className="d-flex flex-column gap-2">
          {sessions.map((s) => (
            <Card key={s.id} className="p-3 d-flex justify-content-between align-items-center flex-row">
              <div>
