...
#             programmer_email TEXT NOT NULL,
...
#     # ---------------------------------------------------------------------
#     # ✅ Ensure programmer_id column exists in qa_sessions
#     # ---------------------------------------------------------------------
#     existing_cols = [row[1] for row in db.execute("PRAGMA table_info(qa_sessions);").fetchall()]
#     if "programmer_id" not in existing_cols:
#         db.execute("ALTER TABLE qa_sessions ADD COLUMN programmer_id TEXT;")
#     if "created_at" not in existing_cols:
#         db.execute("ALTER TABLE qa_sessions ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP;")
...
# #     data = request.get_json()
# #     guide_id = data.get("guideId")
# #     programmer_name = data.get("name")
# #     programmer_email = data.get("email")
# #     if not guide_id or not programmer_name or not programmer_email:
# #         return jsonify({"error": "Missing fields"}), 400

# #     db = get_db()
# #     guide = db.execute("SELECT * FROM users WHERE id=? AND role='guide'", (guide_id,)).fetchone()
# #     if not guide:
# #         return jsonify({"error": "Guide not found"}), 404

# #     request_id = str(uuid.uuid4())
# #     db.execute("INSERT INTO session_requests (id, guide_id, programmer_name, programmer_email) VALUES (?,?,?,?)",
# #                (request_id, guide_id, programmer_name, programmer_email))
# #     db.commit()
# #     return jsonify({"success": True, "message": f"Session requested with {guide['name']}!"})

# @app.route("/api/session-requests/<guide_id>", methods=["GET"])
# def get_session_requests(guide_id):
#     db = get_db()
#     rows = db.execute("SELECT id, programmer_name, programmer_email, status, created_at FROM session_requests WHERE guide_id=? AND status='pending'", (guide_id,)).fetchall()
#     return jsonify({"requests": [dict(r) for r in rows]})
...
#     db = get_db()
#     req = db.execute("SELECT sr.programmer_name, sr.programmer_email, u.id as user_id FROM session_requests sr JOIN users u ON sr.programmer_email=u.email WHERE sr.id=?", (request_id,)).fetchone()
#     if not req:
#         return jsonify({"error":"Session request not found"}), 404

#     db.execute("UPDATE session_requests SET status=? WHERE id=?", (new_status, request_id))
#     message = f"Your session request has been {new_status} by the guide."
#     db.execute("INSERT INTO notifications (id, user_id, message) VALUES (?,?,?)", (str(uuid.uuid4()), req["user_id"], message))
#     db.commit()
#     return jsonify({"success": True, "status": new_status})
...
#     # Get the most recent approved request for this guide
#     req = db.execute("""
#         SELECT sr.programmer_email, u.id as programmer_id
#         FROM session_requests sr
#         JOIN users u ON sr.programmer_email=u.email
#         WHERE sr.guide_id=? AND sr.status='approved'
#         ORDER BY sr.created_at DESC
#         LIMIT 1
#     """, (guide_id,)).fetchone()

#     if not req:
#         return jsonify({"error": "No approved session request found"}), 403
...
#     db = get_db()
#     rows = db.execute("""
#         SELECT id, title, description, meeting_link as meetingLink, created_at as createdAt
#         FROM qa_sessions
#         WHERE guide_id=?
...
# # -------------------------------------------------------------------------
# # Notify Programmer (Console + DB)
# # -------------------------------------------------------------------------
# @app.route("/api/notify-programmer", methods=["POST"])
# def notify_programmer():
#     data = request.get_json()
#     programmer_email = data.get("email")
...
#     db = get_db()
#     user = db.execute("SELECT id FROM users WHERE email=?", (programmer_email,)).fetchone()
#     if user:
#         message = f"Your session request has been {status} by {guide_name}."
#         db.execute("INSERT INTO notifications (id, user_id, message) VALUES (?,?,?)",
#                    (str(uuid.uuid4()), user["id"], message))
#         db.commit()
#     print(f"Notify {data.get('name')} ({programmer_email}): {status} by {guide_name}")
#     return jsonify({"success": True})
